  ...
  const services = [
    {
      ...
      description:
        "Capture your special moments with our professional wedding photography service.",
  ...
  const presets = [
    {
      ...
      pageCount: 20,
      layoutJson: {
        elements: [{ type: "image", x: 0, y: 0, w: 600, h: 400, radius: 0 }],
      },
      bgColor: "#FFFFFF",
      status: "active",
      _seedData: true,
...
async function createTemplatePages(db) {
  ...
  const pages = [
    ...
    },
    {
      ...
      isCover: false,
      layoutJson: {
        elements: [{ type: "image", x: 50, y: 50, w: 300, h: 200 }],
      },
      _seedData: true,
      createdAt: new Date(),
    },
  ...
  const pages = [
    {
      ...
      pageNumber: 1,
      layoutJson: {
        elements: [{ type: "image", x: 100, y: 100, w: 500, h: 400 }],
      },
      images: [],
      _seedData: true,
      updatedAt: new Date(),
...
async function createPhotographersCollection(db) {
  const existing = await db
    ...
    .countDocuments({ _seedData: { $exists: false } });
  if (existing > 0) {
